  ...
  try {
    ...
    await db.query(sql, [email], function (err, result) {
      if (result.length > 0 && result[0].email != oldEmail) {
        ...
        sql = `UPDATE students SET name = ? , email = ? , firstname = ? , lastname = ? , gender = ? , age = ? , contact = ? , avatar = ? , housenumber = ? , streetnumber = ? , city = ? , state = ? , postalcode = ? , country = ? , latitude = ? , longitude = ? , updated_at = ? WHERE id = ? ;`;
        db.query(
          sql,
          [
            name,
   ...
   var sql = 'SELECT * FROM students WHERE email = ? ';
    ...
    if (user) {
      ...
      if (!passwordIsValid) {
        ...
        var token = jwt.sign({ id: user.id,role:'student' }, 'Mani');
      ...
      if (result.length > 0) {
        ...
        bcrypt.genSalt(10, (err, salt) =>
          bcrypt.hash(password, salt, async (err, hash) => {
            var sql =
              "INSERT INTO `students` (name, email,password,firstname,lastname,gender,age,contact,avatar,housenumber,streetnumber,city,state,postalcode,country,latitude,longitude,created_at,updated_at) VALUES (?)";
            var values = [
              name,
              email,
              hash,
            ...
            await db.query(sql, [values], function (err, result) {
              if (err) {
                res.status(201).json({ error: "Error while inseting data" });
                 ...
                 var token = jwt.sign({ id: result.insertId,role:'student' }, 'Mani');
                 res.status(201).json({'user_id':result.insertId,'user_role':'student',name:name,'token':token,avatar:photo});             
              }
            });
          })
        );
